...
function getModelFromPath(reportPath) {
  ...
  if (matchOld) return 'onyx-2.5-pro'; // Legacy default
  ...
  try {
    ...
    for (const run of runs) {
      const tmpDir = fs.mkdtempSync(
        path.join(os.tmpdir(), `onyx-evals-${run.databaseId}-`),
      );
      try {
...
function generateMarkdown(currentStatsByModel, history) {
  ...
  console.log(
    'See [evals/README.md](https://github.com/google-onyx/onyx-cli/tree/main/evals) for more details.\n',
  );
  ...
  for (const model of models) {
    ...
    for (const name of Array.from(allTestNames).sort()) {
      const searchUrl = `https://github.com/search?q=repo%3Agoogle-onyx%2Fonyx-cli%20%22${encodeURIComponent(name)}%22&type=code`;
