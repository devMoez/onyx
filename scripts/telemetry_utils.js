...
// Returns the home directory, respecting ONYX_CLI_HOME
...
export function getJson(url) {
  const tmpFile = path.join(
    os.tmpdir(),
    `onyx-cli-releases-${Date.now()}.json`,
  );
  try {
    const result = spawnSync(
      'curl',
      ['-sL', '-H', 'User-Agent: onyx-cli-dev-script', '-o', tmpFile, url],
...
export async function ensureBinary(
  ...
  const tmpDir = fs.mkdtempSync(
    path.join(os.tmpdir(), 'onyx-cli-telemetry-'),
  );
