...
async function main() {
  const argv = await yargs(hideBin(process.argv))
    ...
    .option('cli-package-name', {
      description:
        'fully qualified package name with scope (e.g @google/onyx-cli)',
      string: true,
      default: '@google/onyx-cli',
    })
    .option('dry-run', {
  ...
  run('git config user.name "onyx-cli-robot"', dryRun);
  run('git config user.email "onyx-cli-robot@google.com"', dryRun);
