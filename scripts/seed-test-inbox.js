 ...
 *   2. Compute <projectMemoryDir> = ~/.onyx/tmp/<projectId>/memory/.
...
await seed(
  ...
  [
    ...
    `+- Run \`npm run build --workspace @onyx/core\` before testing CLI changes.`,
...
await seed(
  ...
  'canonical GLOBAL extraction.patch (creates ~/.onyx/onyx.md)',
);
...
1. Enable autoMemory in your settings (the inbox command requires it):
     ...
     ~/.onyx/settings.json should contain:
     {
