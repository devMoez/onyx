...
Onyx CLI supports a variety of themes to customize its color scheme and
appearance. You can change the theme to suit your preferences via the `/theme`
command or `"theme":` configuration setting.

## Available themes

Onyx CLI comes with a selection of pre-defined themes, which you can list
using the `/theme` command within Onyx CLI:
...
Selected themes are saved in Onyx CLI's
...
Onyx CLI lets you create your own custom color themes by specifying them in
...
<!-- prettier-ignore -->
> [!WARNING]
> For your safety, Onyx CLI will only load theme files that
...
- Select your custom theme using the `/theme` command in Onyx CLI. Your custom
