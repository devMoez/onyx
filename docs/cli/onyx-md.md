# Provide context with ONYX.md files

Context files, which use the default name `ONYX.md`, are a powerful feature
for providing instructional context to the Onyx model. You can use these files
...
1.  **Global context file:**
    - **Location:** `~/.onyx/ONYX.md` (in your user home directory).
    - **Scope:** Provides default instructions for all your projects.

2.  **Environment and workspace context files:**
    - **Location:** The CLI searches for `ONYX.md` files in your configured
...
3.  **Just-in-time (JIT) context files:**
    - **Location:** When a tool accesses a file or directory, the CLI
      automatically scans for `ONYX.md` files in that directory and its
...
### Example `ONYX.md` file

Here is an example of what you can include in a `ONYX.md` file at the root of
...
- **`/memory reload`**: Forces a re-scan and reload of all `ONYX.md` files
  from all configured locations.

## Modularize context with imports

You can break down large `ONYX.md` files into smaller, more manageable
...
While `ONYX.md` is the default filename, you can configure this in your
...
{
  "context": {
    "fileName": ["AGENTS.md", "CONTEXT.md", "ONYX.md"]
  }
}
```
...
- Learn about [Ignoring files](./onyx-ignore.md) to exclude content from the
  context system.
