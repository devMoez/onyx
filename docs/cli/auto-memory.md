...
Auto Memory is an experimental feature that mines your past Onyx CLI sessions
...
Every session you run with Onyx CLI is recorded locally as a transcript. Auto
...
1.  Open your global settings file at `~/.onyx/settings.json`. If you only
    want Auto Memory in one project, edit `.onyx/settings.json` in that
    project instead.
...
3.  Restart Onyx CLI. The flag requires a restart because the extraction
...
1.  **Eligibility scan.** The service indexes recent sessions from
    `~/.onyx/tmp/<project>/chats/`. Sessions are eligible only if they have
...
6.  **Notification.** When a run produces new candidates, Onyx CLI surfaces an
...
- **Promote** a skill to your user (`~/.onyx/skills/`) or workspace
  (`.onyx/skills/`) directory.
...
- The extraction agent runs on a preview Onyx Flash model. Extraction quality
...
- Project or workspace shared instructions in project `ONYX.md` files are not
...
- Explore the [memory management tutorial](./tutorials/memory-management.md) for
  the complementary explicit-memory and `ONYX.md` workflows.
