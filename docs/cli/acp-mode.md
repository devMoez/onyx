...
ACP (Agent Client Protocol) mode is a special operational mode of Onyx CLI
...
ACP-compatible agents across various IDEs. Onyx CLI is an ACP-compatible agent
...
- **Client's role:** The client is responsible for sending requests (for
  example, prompts) and handling responses and notifications from Onyx CLI.
- **Onyx CLI's role:** In ACP mode, Onyx CLI listens for incoming JSON-RPC
...
3.  Onyx CLI connects to the MCP server, discovers the available tools, and
    makes them available to the AI model.
4.  When the model decides to use one of these tools, Onyx CLI sends a tool
