...
Onyx CLI includes a model routing feature that automatically switches to a
...
Onyx CLI supports using a local model for routing decisions. When configured,
Onyx CLI will use a locally-running **Gemma** model to make routing decisions
...
The model used by Onyx CLI is determined by the following order of precedence:
