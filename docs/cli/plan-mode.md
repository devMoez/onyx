...
You can either configure Onyx CLI to start in Plan Mode by default or enter
Plan Mode manually during a session.

### Launch in Plan Mode

To start Onyx CLI directly in Plan Mode by default:
...
To start Plan Mode while using Onyx CLI:

- **Keyboard shortcut:** Press `Shift+Tab` to cycle through approval modes
  (`Default` -> `Auto-Edit` -> `Plan`). Plan Mode is automatically removed from
  the rotation when Onyx CLI is actively processing or showing confirmation
  dialogs.
...
- **Natural Language:** Ask Onyx CLI to "start a plan for...". Onyx CLI
  calls the
  [`enter_plan_mode`](../tools/planning.md#1-enter_plan_mode-enterplanmode) tool
  to switch modes. This tool is not available when Onyx CLI is in
  [YOLO mode](../reference/configuration.md#command-line-arguments).

## How to use Plan Mode

Plan Mode lets you collaborate with Onyx CLI to design a solution before
Onyx CLI takes action.

1.  **Provide a goal:** Start by describing what you want to achieve. Onyx CLI
    will then enter Plan Mode (if it's not already) to research the task.
2.  **Discuss and agree on strategy:** As Onyx CLI analyzes your codebase, it
    ...
    options using [`ask_user`](../tools/ask-user.md). **Onyx CLI will stop and
...
3.  **Review the plan:** Once you've agreed on the strategy, Onyx CLI creates
...
4.  **Approve or iterate:** Onyx CLI will present the finalized plan for your
    formal approval.
...
You can collaborate with Onyx CLI by making direct changes or leaving comments
in the implementation plan. This is often faster and more precise than
describing complex changes in natural language.

1.  **Open the plan:** Press `Ctrl+X` when Onyx CLI presents a plan for
    review.
...
4.  **Review and refine:** Onyx CLI automatically detects the changes, reviews
...
- **Approve a plan:** When Onyx CLI presents a finalized plan, approving it
...
- **Natural language:** Ask Onyx CLI to "exit plan mode" or "stop planning."
...
workflows. You can customize how Onyx CLI plans by using skills, adjusting
safety policies, changing where plans are stored, or adding hooks.

### Custom planning with skills

You can use [Agent Skills](../cli/skills.md) to customize how Onyx CLI
...
- A **"Security Audit"** skill could prompt Onyx CLI to look for specific
  vulnerabilities during codebase exploration.
- A **"Frontend Design"** skill could guide Onyx CLI to use specific UI
  components and accessibility standards in its proposal.

To use a skill in Plan Mode, you can explicitly ask Onyx CLI to "use the
`<skill-name>` skill to plan..." or Onyx CLI may autonomously activate it
...
Tell Onyx CLI it can use these tools in your prompt, for example: _"You can
...
the planning workflow or enforce additional checks when Onyx CLI transitions
...
how Onyx CLI researches and stores plans for your specific domain.
...
When using an [auto model](../reference/configuration.md#model), Onyx CLI
...
Onyx CLI automatically and silently falls back to a faster model to ensure
...
By default, Onyx CLI automatically cleans up old session data, including all
...
When running Onyx CLI in non-interactive environments (such as headless
...
- **Automated implementation:** When exiting Plan Mode to execute the plan,
  Onyx CLI automatically switches to
