...
Onyx CLI provides built-in support for OpenTelemetry, transforming every agent
...
enterprise-wide usage in the cloud, Onyx CLI's observability system provides
the actionable intelligence needed to move from "black box" AI to predictable,
high-performance systems.

## OpenTelemetry integration

Onyx CLI integrates with **[OpenTelemetry]**, a vendor-neutral,
...
After you enable telemetry and run Onyx CLI, you can view your data in the
...
Onyx CLI provides a pre-configured
...
![Onyx CLI Monitoring Dashboard Overview](/docs/assets/monitoring-dashboard-overview.png)

![Onyx CLI Monitoring Dashboard Metrics](/docs/assets/monitoring-dashboard-metrics.png)
...
Onyx CLI includes `session.id`, `installation.id`, `active_approval_mode`, and
`user.email` (when authenticated) as common attributes on all data.

### Logs

Logs provide timestamped records of specific events. Onyx CLI logs events
...
Onyx CLI exports several custom metrics.
