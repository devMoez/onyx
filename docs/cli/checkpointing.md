...
Onyx CLI includes a Checkpointing feature that automatically saves a snapshot
...
1.  **A Git snapshot:** A commit is made in a special, shadow Git repository
    located in your home directory (`~/.onyx/history/<project_hash>`). This
...
`~/.onyx/tmp/<project_hash>/checkpoints`.
