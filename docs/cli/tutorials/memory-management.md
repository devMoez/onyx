...
Control what Onyx CLI knows about you and your projects. In this guide, you'll
learn how to define project-wide rules with `ONYX.md`, teach the agent
...
Onyx CLI is powerful but general. It doesn't know your preferred testing
...
## How to define project-wide rules (ONYX.md)

The most powerful way to control the agent's behavior is through `ONYX.md`
...
1.  In the root of your project, create a file named `ONYX.md`.
...
3.  Start a new session. Onyx CLI will now know these rules automatically.
...
1.  **Global:** `~/.onyx/ONYX.md` (Rules for _every_ project you work on).
2.  **Project Root:** `./ONYX.md` (Rules for the current repository).
3.  **Subdirectory:** `./src/ONYX.md` (Rules specific to the `src` folder).
...
`ONYX.md` files and saved memories), use the `/memory show` command.
...
If you edit a `ONYX.md` file while a session is running, the agent won't know
...
- **Keep it focused:** Avoid adding excessive content to `ONYX.md`. Keep
...
- **Review often:** Periodically check your `ONYX.md` files to remove outdated
  rules.
...
- Explore the [Command reference](../../reference/commands.md) for more
  `/memory` options.
- Read the technical spec for [Project context](../../cli/onyx-md.md).
