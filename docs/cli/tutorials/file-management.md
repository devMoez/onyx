# File management with Onyx CLI

Explore, analyze, and modify your codebase using Onyx CLI. In this guide,
you'll learn how to provide Onyx CLI with files and directories, modify and
create files, and control what Onyx CLI can see.
...
Onyx CLI will generally try to read relevant files, sometimes prompting you
for access (depending on your settings). To ensure that Onyx CLI uses a file,
...
If you _don't_ know the exact file path, you can ask Onyx CLI to find it for
...
Once Onyx CLI has context, you can direct it to make specific edits. The agent
...
Onyx CLI uses the `replace` tool to propose a targeted code change.
...
Onyx CLI uses the `write_file` tool to generate the new file from scratch.

## Review and confirm changes

Onyx CLI prioritizes safety. Before any file is modified, it presents a
...
Onyx CLI uses the `run_shell_command` tool to execute your test runner (for
