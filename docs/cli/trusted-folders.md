...
which projects can use the full capabilities of Onyx CLI. It prevents
...
Once the feature is enabled, the first time you run Onyx CLI from a folder, a
...
Before you make a choice, Onyx CLI performs a **discovery phase** to scan the
...
When a folder is **untrusted**, Onyx CLI runs in a restricted "safe mode" to
...
Granting trust to a folder unlocks the full functionality of Onyx CLI for that
workspace.

## Headless and automated environments

When running Onyx CLI in a headless environment (for example, a CI/CD
...
- **Environment variable:** Set the `ONYX_CLI_TRUST_WORKSPACE=true`
