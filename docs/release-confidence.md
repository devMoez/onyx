...
Onyx CLI. It serves as a checklist and quality gate for release manager to
...
- **Command:** `npx -y @google/onyx-cli@<tag> --version` must return the
...
- **Setup:**
  - [ ] Uninstall any existing global version:
        `npm uninstall -g @google/onyx-cli`
  - [ ] Clear npx cache (optional but recommended): `npm cache clean --force`
  - [ ] Install the preview version: `npm install -g @google/onyx-cli@preview`
  - [ ] Verify version: `onyx --version`
...
- **Basic prompting:**
  - [ ] Run `onyx "Tell me a joke"` and verify a sensible response.
  - [ ] Run in interactive mode: `onyx`. Ask a follow-up question to test
        context.

- **Piped input:**
  - [ ] Run `echo "Summarize this" | onyx` and verify it processes stdin.
...
- **Function calling:**
  - [ ] In interactive mode, ask onyx to "create a file named hello.md with
...
- [ ] Navigate to `go/onyx-cli-offline-evals-dash`.
