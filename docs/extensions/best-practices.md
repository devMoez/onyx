# Onyx CLI extension best practices
...
Developing extensions for Onyx CLI is a lightweight, iterative process. Use
...
Use the `onyx extensions link` command to develop locally without reinstalling
...
### Use `ONYX.md` effectively

Your `ONYX.md` file provides essential context to the model.
...
onyx extensions install github.com/user/repo

# Install the development version
onyx extensions install github.com/user/repo --ref dev
```

### Clean artifacts

When using GitHub Releases, ensure your archives only contain necessary files
(such as `dist/`, `onyx-extension.json`, and `package.json`). Exclude
...
- **Manual verification:** Use `onyx extensions link` to test your extension
...
- **Check the manifest:** Ensure `onyx-extension.json` is in the root
...
- **Restart the CLI:** Extensions are loaded at the start of a session. Restart
  Onyx CLI after making changes to the manifest or linking a new extension.
...
- **Test the command:** Run the server's `command` and `args` directly in your
  terminal to ensure it starts correctly outside of Onyx CLI.
