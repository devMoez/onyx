# Build Onyx CLI extensions

Onyx CLI extensions let you expand the capabilities of Onyx CLI by adding
...
Before you start, ensure you have Onyx CLI installed and a basic understanding
of Node.js.

## Extension features

Extensions offer several ways to customize Onyx CLI. Use this table to decide
...
```bash
onyx extensions new my-first-extension mcp-server
```
...
The manifest file tells Onyx CLI how to load and use your extension.
...
When a user installs this extension, Onyx CLI will prompt them to enter the
...
Link your extension to your Onyx CLI installation for local development.
...
2.  **Link the extension:**
    ...
    The `link` command creates a symbolic link from Onyx CLI extensions
    directory to your development directory. Changes you make are reflected
    immediately.

    ```bash
    onyx extensions link .
    ```

Restart your Onyx CLI session to use the new `fetch_posts` tool. Test it by
asking: "fetch posts".
...
After saving the file, restart Onyx CLI. Run `/fs:grep-code "some pattern"` to
use your new command.

## Step 6: Add a custom `ONYX.md`

Provide persistent context to the model by adding a `ONYX.md` file to your
extension. This is useful for setting behavior or providing essential tool
information.

1.  Create a file named `ONYX.md` in the root of your extension directory:
...
2.  Update your `onyx-extension.json` to load this file:
...
Restart Onyx CLI. The model now has the context from your `ONYX.md` file in
...
Onyx CLI automatically discovers skills bundled with your extension. The model
