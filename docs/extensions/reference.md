...
Create a symbolic link between your development directory and Onyx CLI
...
`commands/` subdirectory. Onyx CLI uses the directory structure to determine
the command name.
...
Extensions can contribute policy rules and safety checkers to Onyx CLI
...
place your `.toml` policy files inside it. Onyx CLI automatically loads all
...
<!-- prettier-ignore -->
> [!WARNING]
> For security, Onyx CLI ignores any `allow` decisions or `yolo`
