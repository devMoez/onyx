...
The `run_shell_command` tool allows the Onyx model to execute commands
...
`settings.json` file or by using the `/settings` command in Onyx CLI.
...
When an interactive command is running, you can send input to it from the Onyx
...
When `run_shell_command` executes a command, it sets the `ONYX_CLI=1`
environment variable in the subprocess's environment. This allows scripts or
tools to detect if they are being run from within Onyx CLI.
