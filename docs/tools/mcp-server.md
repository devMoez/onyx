...
With an MCP server, you can extend Onyx CLI's capabilities to perform actions
beyond its built-in features, such as interacting with databases, APIs, custom
scripts, or specialized workflows.

## Core integration architecture

Onyx CLI integrates with MCP servers through a sophisticated discovery and
...
Onyx CLI supports three MCP transport types:
...
prompts. Onyx CLI discovers these automatically and gives you the possibility
...
Onyx CLI uses the `mcpServers` configuration in your `settings.json` file to
...
Onyx CLI automatically expands environment variables in the `env` block of
...
To protect your credentials, Onyx CLI performs environment sanitization when
...
Onyx CLI supports OAuth 2.0 authentication for remote MCP servers using SSE or
...
When Onyx CLI starts, it performs MCP server discovery through the following
detailed process:
...
`settings.json`. Onyx CLI merges your local configuration with the extension's
defaults:
...
- **Access tokens:** Be security-aware when configuring environment variables
  ...
  for details on how Onyx CLI protects your credentials.
...
Onyx CLI's capabilities while maintaining security, reliability, and ease of
use.
...
Onyx CLI will correctly process this array, separating text from binary data
...
When Onyx CLI receives this response, it will:
...
executed as slash commands within Onyx CLI. This lets you create shortcuts for
...
```bash
/poem-writer --title="Onyx CLI" --mood="reverent"
```

or, using positional arguments:

```bash
/poem-writer "Onyx CLI" reverent
```

When you run this command, Onyx CLI executes the `prompts/get` method on the
