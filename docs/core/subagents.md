...
Subagents are specialized agents that operate within your main Onyx CLI
...
Onyx CLI's main agent is instructed to use specialized subagents when a task
...
Onyx CLI comes with the following built-in subagents:
...
- **Name:** `cli_help`
- **Purpose:** Get expert knowledge about Onyx CLI itself, its commands,
...
server is bundled with Onyx CLI and launched automatically — no separate
...
The first time the browser agent is invoked, Onyx CLI displays a consent
...
Subagent tool isolation moves Onyx CLI away from a single global tool
...
Onyx CLI can also delegate tasks to remote subagents using the Agent-to-Agent
(A2A) protocol.
