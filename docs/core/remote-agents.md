...
Onyx CLI supports connecting to remote subagents using the Agent-to-Agent
(A2A) protocol. This allows Onyx CLI to interact with other agents, expanding
its capabilities by delegating tasks to remote services.

Onyx CLI can connect to any compliant A2A agent. You can find samples of A2A
...
Onyx CLI routes traffic to remote agents through an HTTP/HTTPS proxy if one is
...
Many remote agents require authentication. Onyx CLI supports several
...
Onyx CLI supports the following authentication types:
...
use, Onyx CLI opens your browser for sign-in and persists the resulting tokens
...
When Onyx CLI loads a remote agent, it validates your auth configuration
...
When connecting to a remote agent, Onyx CLI first fetches the agent card
...
Users can manage subagents using the following commands within Onyx CLI:
...
<!-- prettier-ignore -->
> [!TIP]
> You can use the `@cli_help` agent within Onyx CLI for assistance
