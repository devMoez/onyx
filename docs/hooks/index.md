...
Hooks are scripts or programs that Onyx CLI executes at specific points in the
...
Onyx CLI waits for all matching hooks to complete before continuing.
...
Hooks are triggered by specific events in Onyx CLI's lifecycle.
...
3. **Debug via Stderr**: Use `stderr` for **all** logging and debugging (for
   example, `echo "debug" >&2`). Onyx CLI captures `stderr` but never attempts
   to parse it as JSON.

#### Exit codes

Onyx CLI uses exit codes to determine the high-level outcome of a hook
execution:
...
Onyx CLI **fingerprints** project hooks. If a hook's name or command changes
