# Writing hooks for Onyx CLI

This guide will walk you through creating hooks for Onyx CLI, from a simple
...
- Onyx CLI installed and configured
...
```bash
mkdir -p .onyx/hooks
cat > .onyx/hooks/log-tools.sh << 'EOF'
#!/usr/bin/env bash
...
# Log to file
echo "[$(date)] Tool executed: $tool_name" >> .onyx/tool-log.txt
...
chmod +x .onyx/hooks/log-tools.sh
```

**Windows (PowerShell)**

```powershell
New-Item -ItemType Directory -Force -Path ".onyx\hooks"
@"
# Read hook input from stdin
...
# Log to file
"[`$(Get-Date -Format 'o')] Tool executed: `$toolName" | Out-File -FilePath ".onyx\tool-log.txt" -Append -Encoding utf8

# Return success with empty JSON
"{}"
"@ | Out-File -FilePath ".onyx\hooks\log-tools.ps1" -Encoding utf8
```

## Exit Code Strategies

There are two ways to control or block an action in Onyx CLI:
        ...
        "hooks": [
          {
            "name": "intent-filter",
            "type": "command",
            "command": "node .onyx/hooks/filter-tools.js"
          }
        ]
      }
    ]
...
### Configuration (`.onyx/settings.json`)
        ...
        "hooks": [
          {
            "name": "init",
            "type": "command",
            "command": "node .onyx/hooks/init.js"
          }
        ]
      }
    ],
        ...
        "hooks": [
          {
            "name": "memory",
            "type": "command",
            "command": "node .onyx/hooks/inject-memories.js"
          }
        ]
      }
    ],
        ...
        "hooks": [
          {
            "name": "filter",
            "type": "command",
            "command": "node .onyx/hooks/rag-filter.js"
          }
        ]
      }
    ],
        ...
        "hooks": [
          {
            "name": "security",
            "type": "command",
            "command": "node .onyx/hooks/security.js"
          }
        ]
      }
    ],
        ...
        "hooks": [
          {
            "name": "record",
            "type": "command",
            "command": "node .onyx/hooks/record.js"
          }
        ]
      }
    ],
        ...
        "hooks": [
          {
            "name": "validate",
            "type": "command",
            "command": "node .onyx/hooks/validate.js"
          }
        ]
      }
    ],
        ...
        "hooks": [
          {
            "name": "save",
            "type": "command",
            "command": "node .onyx/hooks/consolidate.js"
          }
        ]
      }
    ]
...
[Onyx CLI extension](../extensions/index.md). This provides version control,
