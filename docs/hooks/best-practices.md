...
Onyx CLI uses exit codes for high-level flow control:
...
> changed (for example, by a `git pull`), its identity changes. Onyx CLI will
...
Hooks inherit the environment of Onyx CLI process, which may include sensitive
API keys. Onyx CLI provides a
...
Prevent denial-of-service (hanging agents) by enforcing timeouts. Onyx CLI
