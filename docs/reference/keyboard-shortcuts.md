# Onyx CLI keyboard shortcuts

Onyx CLI ships with a set of default keyboard shortcuts for editing input,
...
| `app.focusShellInput`         | Move focus from Onyx to the active shell.                                                                                                        | `Tab`              |
| `app.unfocusShellInput`       | Move focus from the shell back to Onyx.                                                                                                          | `Shift+Tab`        |
...
| `background.unfocus`        | Move focus from background shell to Onyx.                        | `Shift+Tab` |
| `background.unfocusList`    | Move focus from background shell list to Onyx.                   | `Tab`       |
...
`keybindings.json` file in your home onyx directory (typically
`~/.onyx/keybindings.json`).
...
When vim mode is enabled with `/vim` or `general.vimMode: true`, Onyx CLI
...
- On [Windows Terminal](https://en.wikipedia.org/wiki/Windows_Terminal):
  ...
  - `shift+tab`
    [is not supported](https://github.com/google-onyx/onyx-cli/issues/20314)
