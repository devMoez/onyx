...
Onyx CLI uses tools to interact with your local environment, access
...
## How to use Onyx CLI's tools

Tools are generally invoked automatically by Onyx CLI when it needs to perform
an action. However, you can also trigger specific tools manually using shorthand
syntax.

### Automatic execution and security

When the model wants to use a tool, Onyx CLI evaluates the request against its
security policies.
...
You can extend Onyx CLI with custom tools by configuring
