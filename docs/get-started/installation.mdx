...
# Onyx CLI installation, execution, and releases

This document provides an overview of Onyx CLI's system requirements,
...
- **Location:**
  [Onyx Code Assist supported locations](https://developers.google.com/onyx-code-assist/resources/available-locations#americas)
- **Internet connection required**

## Install Onyx CLI

We recommend most users install Onyx CLI using one of the following
installation methods. Note that Onyx CLI comes pre-installed on
...
For most users, we recommend running Onyx CLI with the `onyx` command:
...
You can also run Onyx CLI using one of the following advanced methods:

<Tabs>
  <TabItem label="npx">

  Run instantly with npx. You can run Onyx CLI without permanent installation.
  ...
  For security and isolation, Onyx CLI can be run inside a container. This is
  ...
  - **Directly from the registry:** You can run the published sandbox image
    ...
    docker run --rm -it us-docker.pkg.dev/onyx-code-dev/onyx-cli/sandbox:0.42.0-nightly.20260428.g59b2dea0e
    ```
  - **Using the `--sandbox` flag:** If you have Onyx CLI installed locally
    (using the standard installation described above), you can instruct it to run
    inside the sandbox container.
    ```bash
    onyx --sandbox -y -p "your prompt here"
    ```
  ...
  - **Production-like mode (linked package):** This method simulates a global
    ...
    # Now you can run your local version using the `onyx` command
    onyx
    ```
...
Onyx CLI has three release channels: stable, preview, and nightly. For most
