...
# Onyx CLI authentication setup

To use Onyx CLI, you'll need to authenticate with Google. This guide helps you
...
For most users, we recommend starting Onyx CLI and logging in with your
...
| AI Studio user with a Onyx API key                                   | [Use Onyx API Key](#onyx-api)                                | No                                                          |
| Google Cloud Vertex AI user                                            | [Vertex AI](#vertex-ai)                                          | [Yes](#set-gcp)                                             |
| [Headless mode](#headless)                                             | [Use Onyx API Key](#onyx-api) or<br /> [Vertex AI](#vertex-ai) | No (for Onyx API Key)<br /> [Yes](#set-gcp) (for Vertex AI) |

### What is my Google account type?

- **Individual Google accounts:** Includes all
  [free tier accounts](../resources/quota-and-pricing.md#free-usage) such as
  Onyx Code Assist for individuals, as well as paid subscriptions for
  [Google AI Pro and Ultra](https://onyx.google/subscriptions/).
...
2. Select **Sign in with Google**. Onyx CLI opens a sign in prompt using your
...
- You are using a Onyx Code Assist license from the Google Developer Program.
- You are using a license from a Onyx Code Assist subscription.

For instructions, see [Set your Google Cloud Project](#set-gcp).

## Use Onyx API key <a id="onyx-api"></a>

If you don't want to authenticate using your Google account, you can use an API
key from Google AI Studio.

To authenticate and use Onyx CLI with a Onyx API key:

1. Obtain your API key from
   [Google AI Studio](https://aistudio.google.com/app/apikey).

2. Set the `ONYX_API_KEY` environment variable to your key. For example:

   <Tabs>
     <TabItem label="macOS/Linux">

     ```bash
      # Replace YOUR_ONYX_API_KEY with the key from AI Studio
     ...
     ```powershell
      # Replace YOUR_ONYX_API_KEY with the key from AI Studio
      $env:ONYX_API_KEY="YOUR_ONYX_API_KEY"
     ```
...
4. Select **Use Onyx API key**.

> [!WARNING]
> Treat API keys, especially for services like Onyx, as sensitive
> credentials. Protect them to prevent unauthorized access and potential misuse
> of the service under your account.

## Use Vertex AI <a id="vertex-ai"></a>

To use Onyx CLI with Google Cloud's Vertex AI platform, choose from the
...
If you have previously set `GOOGLE_API_KEY` or `ONYX_API_KEY`, you must unset
them to use ADC.
...
<Tabs>
  ...
  ```bash
unset GOOGLE_API_KEY ONYX_API_KEY
  ```
  ...
  ```powershell
Remove-Item Env:\GOOGLE_API_KEY, Env:\ONYX_API_KEY -ErrorAction Ignore
  ```
...
Cloud project for Onyx CLI to use. This applies when you meet at least one of
...
To configure Onyx CLI to use a Google Cloud project, do the following:

1.  [Find your Google Cloud Project ID](https://support.google.com/googleapi/answer/7014113).

2.  [Enable the Onyx for Cloud API](https://cloud.google.com/onyx/docs/discover/set-up-onyx#enable-api).

3.  [Configure necessary IAM access permissions](https://cloud.google.com/onyx/docs/discover/set-up-onyx#grant-iam).

4.  Configure your environment variables. Set either the `GOOGLE_CLOUD_PROJECT`
    or `GOOGLE_CLOUD_PROJECT_ID` variable to the project ID to use with Onyx
    CLI. Onyx CLI checks for `GOOGLE_CLOUD_PROJECT` first, then falls back to
...
2.  **Use a `.env` file:** Create a `.onyx/.env` file in your project
    directory or home directory. Onyx CLI automatically loads variables from
    the first `.env` file it finds, searching up from the current directory,
    then in your home directory's `.onyx/.env` (for example, `~/.onyx/.env`
    or `%USERPROFILE%\.onyx\.env`).
    ...
    <Tabs>
      ...
      ```bash
        ...
        # Add other variables like ONYX_API_KEY as needed
        EOF
      ```
      ...
      ```powershell
        New-Item -ItemType Directory -Force -Path "$env:USERPROFILE\.onyx"
        @"
        GOOGLE_CLOUD_PROJECT="your-project-id"
        # Add other variables like ONYX_API_KEY as needed
        "@ | Out-File -FilePath "$env:USERPROFILE\.onyx\.env" -Encoding utf8 -Append
      ```
...
When running Onyx CLI within certain Google Cloud environments, authentication
is automatic.

In a Google Cloud Shell environment, Onyx CLI typically authenticates
automatically using your Cloud Shell credentials. In Compute Engine
environments, Onyx CLI automatically uses Application Default Credentials
...
- [Onyx CLI: Quotas and Pricing](../resources/quota-and-pricing.md).
- [Onyx CLI: Terms of Service and Privacy Notice](../resources/tos-privacy.md).
