# Get started with Onyx CLI

Welcome to Onyx CLI! This guide will help you install, configure, and start
using Onyx CLI to enhance your workflow right from your terminal.

## Quickstart: Install, authenticate, configure, and use Onyx CLI

Onyx CLI brings the power of advanced language models directly to your command
line interface. As an AI-powered assistant, Onyx CLI can help you with a
...
[Onyx CLI Installation](./installation.mdx).

## Authenticate

To begin using Onyx CLI, you must authenticate with a Google service. In most
...
[Onyx CLI Authentication Setup](./authentication.mdx).
...
[Onyx CLI Configuration](../reference/configuration.md).

## Use

Once installed and authenticated, you can start using Onyx CLI by issuing
...
You can use Onyx CLI to automate file management tasks that require visual
analysis. In this example, Onyx CLI renames images based on their actual
subject matter.
...
Onyx CLI is effective for rapid codebase exploration. The following example
shows how to ask Onyx CLI to fetch, analyze, and summarize a remote project.

Scenario: You want to understand how a popular open-source utility works by
inspecting its code, not just its README.

Give Onyx CLI the following prompt:
...
Onyx CLI returns an explanation based on the actual source code:
...
Onyx CLI can process and transform data across multiple files. Use this
...
Result: Onyx CLI reads each file and then asks for permission to write a new
file. Provide your permission and Onyx CLI provides the combined data:
...
Onyx CLI can generate boilerplate code and tests based on your existing
...
Result: Onyx CLI asks for permission to write a new file and creates a test
for your login page.
