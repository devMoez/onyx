...
Onyx CLI can integrate with your IDE to provide a more seamless and
...
There are two primary ways to integrate Onyx CLI with an IDE:

1.  **VS Code companion extension**: Install the "Onyx CLI Companion"
...
2.  **Agent Client Protocol (ACP)**: An open protocol for interoperability
    ...
    discovery and installation of compatible agents like Onyx CLI.

## VS Code companion extension

The **Onyx CLI Companion extension** grants Onyx CLI direct access to your
...
- **VS Code commands:** You can access Onyx CLI features directly from the VS
  Code Command Palette (`Cmd+Shift+P` or `Ctrl+Shift+P`):
  - `Onyx CLI: Run`: Starts a new Onyx CLI session in the integrated
    terminal.
  - `Onyx CLI: Accept Diff`: Accepts the changes in the active diff editor.
  - `Onyx CLI: Close Diff Editor`: Rejects the changes and closes the active
    diff editor.
  - `Onyx CLI: View Third-Party Notices`: Displays the third-party notices for
    the extension.
...
When you run Onyx CLI inside a supported editor, it will automatically detect
...
manually, you can run the following command inside Onyx CLI:
...
<!-- prettier-ignore -->
> [!NOTE]
> The "Onyx CLI Companion" extension may appear towards the bottom of
...
When enabled, Onyx CLI will automatically attempt to connect to the IDE
companion extension.
...
- Open the Command Palette and run **Onyx CLI: Accept Diff**.
...
- Open the Command Palette and run **Onyx CLI: Close Diff Editor**.
...
Onyx CLI is officially available in the **ACP Agent Registry**. This allows
you to install and update Onyx CLI directly within supporting IDEs and
...
Onyx CLI is an ACP-compatible agent available in the ACP Agent Registry.
...
Any other IDE that supports the ACP Agent Registry can install Onyx CLI
directly through their in-built registry features.

## Using with sandboxing

If you are using Onyx CLI within a sandbox, be aware of the following:
...
- **On macOS:** The IDE integration requires network access to communicate with
  ...
  network access.
- **In a Docker container:** If you run Onyx CLI inside a Docker (or Podman)
...
- **Message:**
  ...
  - **Solution:**
    1.  Make sure you have installed the **Onyx CLI Companion** extension in
...
- **Message:**
  `🔴 Disconnected: Directory mismatch. Onyx CLI is running in a different location than the open workspace in [IDE Name]. Please run the CLI from one of the following directories: [List of directories]`
...
- **Message:**
  `IDE integration is not supported in your current environment. To use this feature, run Onyx CLI in one of these supported IDEs: [List of IDEs]`
  - **Cause:** You are running Onyx CLI in a terminal or environment that is
    not a supported IDE.
  - **Solution:** Run Onyx CLI from the integrated terminal of a supported
    IDE, like Antigravity or VS Code.

- **Message:**
  `No installer is available for IDE. Please install Onyx CLI Companion extension manually from the marketplace.`
  - **Cause:** You ran `/ide install`, but the CLI does not have an automated
    installer for your specific IDE.
  - **Solution:** Open your IDE's extension marketplace, search for "Onyx CLI
    Companion", and
