...
"uninstall" the CLI, you must clear this cache, which will remove onyx-cli and
...
`npm install -g @google/onyx-cli`), use the `npm uninstall` command with the
`-g` flag to remove it.
...
`brew install onyx-cli`), use the `brew uninstall` command to remove it.
...
`sudo port install onyx-cli`), use the `port uninstall` command to remove it.

```bash
sudo port uninstall onyx-cli
```
