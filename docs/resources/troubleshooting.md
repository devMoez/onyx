...
- **Error: "Operation not permitted", "Permission denied", or similar.**
  - **Cause:** When sandboxing is enabled, Onyx CLI may attempt operations
...
- **Onyx CLI is not running in interactive mode in "CI" environments**
  - **Issue:** Onyx CLI does not enter interactive mode (no prompt appears) if
  ...
  - **Cause:** The `is-in-ci` package checks for the presence of `CI`,
    ...
    non-interactive, which prevents Onyx CLI from starting in its interactive
    mode.
...
- **Warning: `npm WARN deprecated node-domexception@1.0.0` or
  ...
  - **Cause:** These warnings occur because some dependencies (or their
    ...
    versions. Since Onyx CLI requires Node.js 20 or higher, the platform's
...
Onyx CLI uses specific exit codes to indicate the reason for termination. This
