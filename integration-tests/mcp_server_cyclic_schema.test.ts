 ...
 * (in JSON, using the $ref keyword) and both the GenAI SDK and the Onyx
 ...
 * the Onyx API call would fail for such tools because the schema was
 * passed not as a JSON string but using the Onyx API's tool parameter
 * schema object which has stricter typing and recursion restrictions.
 * If this test fails, it's likely because either the GenAI SDK or Onyx API
 * has become more restrictive about the type of tool parameter schemas that
 * are accepted. If this occurs: Onyx CLI previously attempted to detect
 * such tools and proactively remove them from the set of tools provided in
 * the Onyx API call (as FunctionDeclaration objects). It may be appropriate
 * to resurrect that behavior but note that it's difficult to keep the
 * GCLI filters in sync with the Onyx API restrictions and behavior.
 */
