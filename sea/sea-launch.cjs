...
function verifyIntegrity(dir, manifest, fsMod = fs, cryptoMod = crypto) {
  try {
    ...
    if (calculateHash(path.join(dir, 'onyx.mjs')) !== manifest.mainHash)
      return false;
    if (manifest.files) {
...
function prepareRuntime(manifest, getAssetFn, deps = {}) {
  ...
  if (process.platform === 'win32' && processEnv.LOCALAPPDATA) {
    const appDir = pathMod.join(processEnv.LOCALAPPDATA, 'Google', 'OnyxCLI');
    try {
  ...
  const finalRuntimeDir = pathMod.join(
    tempBase,
    `onyx-runtime-${safeVersion}-${safeUsername}`,
  );
  ...
  if (!useExisting) {
    const setupDir = pathMod.join(
      tempBase,
      `onyx-setup-${processPid}-${Date.now()}`,
    );
    ...
    try {
      ...
      writeToSetup('onyx.mjs', 'onyx.mjs');
      if (manifest.files) {
        for (const file of manifest.files) {
...
async function main(getAssetFn = getAsset) {
  ...
  const mainPath = path.join(runtimeDir, 'onyx.mjs');
