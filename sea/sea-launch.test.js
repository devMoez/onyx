...
describe('sea-launch', () => {
  ...
  describe('verifyIntegrity', () => {
    it('returns true for matching hashes', () => {
      ...
      const mockFs = {
        openSync: vi.fn((p) => {
          if (p.endsWith('onyx.mjs')) return 10;
  ...
  describe('prepareRuntime', () => {
    ...
    it('reuses existing runtime if secure and valid', () => {
      ...
      expect(runtime).toContain('onyx-runtime-1.0.0-user');
    ...
    it('recreates runtime if existing has wrong owner', () => {
      ...
      expect(deps.fs.rmSync).toHaveBeenCalledWith(
        expect.stringContaining('onyx-runtime'),
        expect.anything(),
      );
      expect(deps.fs.mkdirSync).toHaveBeenCalledWith(
        expect.stringContaining('onyx-setup'),
        expect.anything(),
      );
    });
    ...
    it('handles rename race condition: uses target if secure and valid', () => {
      ...
      expect(runtime).toContain('onyx-runtime');
      expect(deps.fs.rmSync).toHaveBeenCalledWith(
        expect.stringContaining('onyx-setup'),
        expect.anything(),
      );
    ...
    it('uses LOCALAPPDATA on Windows if available', () => {
      ...
      expect(deps.fs.mkdirSync).toHaveBeenCalledWith(
        'C:\\Users\\User\\AppData\\Local\\Google\\OnyxCLI',
    ...
    it('falls back to tmpdir on Windows if LOCALAPPDATA is missing', () => {
      ...
      expect(runtime).not.toContain('Google\\OnyxCLI');
    ...
    it('falls back to tmpdir on Windows if mkdir fails', () => {
      ...
      const deps = {
        fs: {
          existsSync: vi.fn().mockReturnValue(false),
          mkdirSync: vi.fn((p) => {
            if (typeof p === 'string' && p.includes('Google\\OnyxCLI')) {
      ...
      expect(runtime).toContain('C:\\Temp');
      expect(deps.fs.mkdirSync).toHaveBeenCalledWith(
        expect.stringContaining('Google\\OnyxCLI'),
        expect.anything(),
      );
