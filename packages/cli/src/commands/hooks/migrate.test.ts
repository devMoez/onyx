...
describe('migrate command', () => {
  ...
  beforeEach(() => {
    ...
    mockedLoadSettings.mockReturnValue({
      ...
      workspace: { path: '/test/project/.onyx' },
    });
  });
  ...
  it('should migrate hooks from settings.json when it exists', async () => {
    ...
    expect(debugLoggerLogSpy).toHaveBeenCalledWith(
      '✓ Hooks successfully migrated to .onyx/settings.json',
    );
  });
  ...
  it('should replace $CLAUDE_PROJECT_DIR with $ONYX_PROJECT_DIR', async () => {
    const claudeSettings = {
      hooks: {
        PreToolUse: [
          {
  ...
  it('should merge with existing Onyx hooks', async () => {
    const claudeSettings = {
      hooks: {
        PreToolUse: [
          {
