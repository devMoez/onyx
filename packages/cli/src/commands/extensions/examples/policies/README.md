...
- `onyx-extension.json`: The manifest file.
- `policies/`: Contains the `.toml` policy files.

## How to use

1.  Link this extension to your local Onyx CLI installation:

    ```bash
    onyx extensions link packages/cli/src/commands/extensions/examples/policies
    ```

2.  Restart your Onyx CLI session.
...
For security, Onyx CLI ignores any `allow` decisions or `yolo` mode
