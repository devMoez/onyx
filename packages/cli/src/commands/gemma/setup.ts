...
async function handleSetup(argv: SetupArgs): Promise<number> {
  ...
  try {
    ...
    log(chalk.dim('    User (~/.onyx/settings.json): autoStartServer'));
    log(
      chalk.dim('    Workspace (.onyx/settings.json): enabled, classifier'),
    );
    ...
    logError(
      '  You can manually add the configuration to ~/.onyx/settings.json',
    );
  }
