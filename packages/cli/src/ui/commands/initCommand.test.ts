...
import type { SubmitPromptActionReturn } from '@onyx/core';
...
describe('initCommand', () => {
  ...
  it('should inform the user if onyx.md already exists', async () => {
    ...
    expect(result).toEqual({
      ...
      messageType: 'info',
      content:
        'A onyx.md file already exists in this directory. No changes were made.',
    });
    // Assert: Ensure no file was written
    expect(fs.writeFileSync).not.toHaveBeenCalled();
  });

  it('should create onyx.md and submit a prompt if it does not exist', async () => {
    ...
    expect(fs.writeFileSync).toHaveBeenCalledWith(onyxMdPath, '', 'utf8');

    // Assert: Check that an informational message was added to the UI
    expect(mockContext.ui.addItem).toHaveBeenCalledWith(
      {
        type: 'info',
        text: 'Empty onyx.md created. Now analyzing the project to populate it.',
      },
      expect.any(Number),
    );
    ...
    expect(result.content).toContain(
      'You are an AI agent that brings the power of Onyx',
    );
  });
