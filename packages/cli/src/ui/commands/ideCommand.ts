...
import {
  ...
  IDEConnectionStatus,
  ideContextStore,
  ONYX_CLI_COMPANION_EXTENSION_NAME,
} from '@onyx/core';
    ...
    return {
      ...
      action: (): SlashCommandActionReturn =>
        ({
          type: 'message',
          messageType: 'error',
          content: `IDE integration is not supported in your current environment. To use this feature, run Onyx CLI in one of these supported IDEs: Antigravity, VS Code, or VS Code forks.`,
        }) as const,
    };
  }
    ...
    action: async (context) => {
      ...
      if (!installer) {
        context.ui.addItem(
          {
            type: 'error',
            text: `No installer is available for ${ideClient.getDetectedIdeDisplayName()}. Please install the '${ONYX_CLI_COMPANION_EXTENSION_NAME}' extension manually from the marketplace.`,
          },
          Date.now(),
        );
        return;
