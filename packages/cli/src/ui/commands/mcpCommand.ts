...
import type { MessageActionReturn } from '@onyx/core';
import {
  DiscoveredMCPTool,
...
const authCommand: SlashCommand = {
  ...
  action: async (
    ...
    try {
      ...
      const { MCPOAuthProvider } = await import('@onyx/core');
...
const reloadCommand: SlashCommand = {
  ...
  action: async (
    ...
    // Update the client with the new tools
    const onyxClient = agentContext.onyxClient;
    if (onyxClient?.isInitialized()) {
      await onyxClient.setTools();
    }
...
async function handleEnableDisable(
  ...
  if (agentContext.onyxClient?.isInitialized())
    await agentContext.onyxClient.setTools();
  context.ui.reloadCommands();
