...
import { escapeShellArg, isWindows, type ShellType } from '@onyx/core';
...
export const SessionSummaryDisplay: React.FC<SessionSummaryDisplayProps> = ({
  ...
  const footerSessionId =
    ...
    !escapedSessionId.startsWith("'")
      ? `"${escapedSessionId}"`
      : escapedSessionId;
  let footer = `To resume this session: onyx --resume ${footerSessionId}`;

  if (worktreeSettings) {
    footer =
      `To resume work in this worktree: cd ${escapeShellArg(worktreeSettings.path, shell)} && onyx --resume ${footerSessionId}\n` +
