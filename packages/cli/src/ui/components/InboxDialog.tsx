...
const SKILL_DESTINATION_CHOICES: DestinationChoice[] = [
  {
    destination: 'global',
    label: 'Global',
    description: '~/.onyx/skills — available in all projects',
  },
  {
    destination: 'project',
    label: 'Project',
    description: '.onyx/skills — available in this workspace',
  },
];
...
// project (private MEMORY.md and sibling files, plus ~/.onyx/onyx.md),
...
function getSkillOriginTag(filePath: string): string {
  ...
  if (normalizedPath.includes('/.onyx/skills/')) {
    ...
    if (
      homeDirs.some((homeDir) =>
        normalizedPath.startsWith(`${homeDir}/.onyx/skills/`),
      )
    ) {
      return 'Global';
    }
