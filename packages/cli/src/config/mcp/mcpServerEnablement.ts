...
import { Storage, coreEvents } from '@onyx/core';
...
export async function canLoadServer(
  ...
  if (config.enablement?.isSessionDisabled(normalizedId)) {
    return {
      allowed: false,
      reason: `Server '${serverId}' is disabled for this session. Run 'onyx mcp enable ${serverId} --session' to clear.`,
      blockType: 'session',
    };
  }
  ...
  if (
    ...
    return {
      allowed: false,
      reason: `Server '${serverId}' is disabled. Run 'onyx mcp enable ${serverId}' to enable.`,
...
export class McpServerEnablementManager {
  ...
  constructor() {
    this.configDir = Storage.getGlobalOnyxDir();
