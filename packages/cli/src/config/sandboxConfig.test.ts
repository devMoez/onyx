...
import { getPackageJson } from '@onyx/core';
...
describe('loadSandboxConfig', () => {
  ...
  beforeEach(() => {
    ...
    delete process.env['ONYX_SANDBOX'];
  ...
  describe('with ONYX_SANDBOX environment variable', () => {
    it('should use docker if ONYX_SANDBOX=docker and it exists', async () => {
      process.env['ONYX_SANDBOX'] = 'docker';
    ...
    it('should throw if ONYX_SANDBOX is an invalid command', async () => {
      process.env['ONYX_SANDBOX'] = 'invalid-command';
    ...
    it('should throw if ONYX_SANDBOX command does not exist', async () => {
      process.env['ONYX_SANDBOX'] = 'docker';
      mockedCommandExistsSync.mockReturnValue(false);
      await expect(loadSandboxConfig({}, {})).rejects.toThrow(
        "Missing sandbox command 'docker' (from ONYX_SANDBOX)",
      );
    });

    it('should use lxc if ONYX_SANDBOX=lxc and it exists', async () => {
      process.env['ONYX_SANDBOX'] = 'lxc';
      mockedCommandExistsSync.mockReturnValue(true);
      const config = await loadSandboxConfig({}, {});
      expect(config).toEqual({
        enabled: true,
    ...
    it('should throw if ONYX_SANDBOX=lxc but lxc command does not exist', async () => {
      process.env['ONYX_SANDBOX'] = 'lxc';
      mockedCommandExistsSync.mockReturnValue(false);
      await expect(loadSandboxConfig({}, {})).rejects.toThrow(
        "Missing sandbox command 'lxc' (from ONYX_SANDBOX)",
      );
    });
  });
  ...
  describe('with sandbox: true', () => {
    ...
    it('should throw if sandbox: true but no command is found', async () => {
      ...
      await expect(loadSandboxConfig({}, { sandbox: true })).rejects.toThrow(
        'ONYX_SANDBOX is true but failed to determine command for sandbox; ' +
          'install docker or podman or specify command in ONYX_SANDBOX',
      );
    });
  });
  ...
  describe("with sandbox: 'command'", () => {
    ...
    it('should throw if the specified command does not exist', async () => {
      ...
      await expect(
        ...
        "Missing sandbox command 'podman' (from ONYX_SANDBOX)",
      );
    });
  ...
  describe('image configuration', () => {
    it('should use image from ONYX_SANDBOX_IMAGE env var if set', async () => {
      process.env['ONYX_SANDBOX_IMAGE'] = 'env/image';
      process.env['ONYX_SANDBOX'] = 'docker';
      mockedCommandExistsSync.mockReturnValue(true);
      const config = await loadSandboxConfig({}, {});
      expect(config).toEqual({
  ...
  describe('with sandbox: runsc (gVisor)', () => {
    ...
    it('should use runsc via ONYX_SANDBOX environment variable', async () => {
      process.env['ONYX_SANDBOX'] = 'runsc';
      const config = await loadSandboxConfig({}, {});
    ...
    it('should prioritize ONYX_SANDBOX over CLI and settings', async () => {
      process.env['ONYX_SANDBOX'] = 'runsc';
      const config = await loadSandboxConfig(
    ...
    it('should throw if runsc binary not found', async () => {
      ...
      await expect(loadSandboxConfig({}, { sandbox: 'runsc' })).rejects.toThrow(
        "Missing sandbox command 'runsc' (from ONYX_SANDBOX)",
      );
    });
