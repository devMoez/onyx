...
function getSandboxCommand(
  ...
  const environmentConfiguredSandbox =
    process.env['ONYX_SANDBOX']?.toLowerCase().trim() ?? '';
  sandbox =
  ...
  if (typeof sandbox === 'string' && sandbox) {
    ...
    if (sandbox !== 'windows-native' && !commandExists.sync(sandbox)) {
      throw new FatalSandboxError(
        `Missing sandbox command '${sandbox}' (from ONYX_SANDBOX)`,
      );
    }
  ...
  // ONYX_SANDBOX=lxc or sandbox: "lxc" in settings to enable it.
}
...
export async function loadSandboxConfig(
  ...
  const image =
    process.env['ONYX_SANDBOX_IMAGE'] ??
    process.env['ONYX_SANDBOX_IMAGE_DEFAULT'] ??
    customImage ??
