...
describe('validateNonInterActiveAuth', () => {
  let originalEnvOnyxApiKey: string | undefined;
  ...
  beforeEach(() => {
    originalEnvOnyxApiKey = process.env['ONYX_API_KEY'];
  ...
  afterEach(() => {
    if (originalEnvOnyxApiKey !== undefined) {
      process.env['ONYX_API_KEY'] = originalEnvOnyxApiKey;
    } else {
      delete process.env['ONYX_API_KEY'];
    }
  ...
  it('uses USE_ONYX if ONYX_API_KEY is set', async () => {
    process.env['ONYX_API_KEY'] = 'fake-key';
    const nonInteractiveConfig = createLocalMockConfig({});
    await validateNonInteractiveAuth(
      undefined,
      undefined,
  ...
  it('uses USE_VERTEX_AI if both ONYX_API_KEY and GOOGLE_GENAI_USE_VERTEXAI are set', async () => {
    process.env['ONYX_API_KEY'] = 'fake-key';
    process.env['GOOGLE_GENAI_USE_VERTEXAI'] = 'true';
    process.env['GOOGLE_CLOUD_PROJECT'] = 'test-project';
    process.env['GOOGLE_CLOUD_LOCATION'] = 'us-central1';
  ...
  it('uses USE_ONYX if GOOGLE_GENAI_USE_VERTEXAI is false, ONYX_API_KEY is set, and project/location are available', async () => {
  ...
  it('succeeds if effectiveAuthType matches enforcedType', async () => {
    mockSettings.merged.security.auth.enforcedType = AuthType.USE_ONYX;
    process.env['ONYX_API_KEY'] = 'fake-key';
    const nonInteractiveConfig = createLocalMockConfig({});
    await validateNonInteractiveAuth(
      undefined,
  ...
  it('exits if configuredAuthType does not match enforcedType', async () => {
    ...
    expect(debugLoggerErrorSpy).toHaveBeenCalledWith(
      "The enforced authentication type is 'oauth-personal', but the current type is 'onyx-api-key'. Please re-authenticate with the correct type.",
    );
    expect(processExitSpy).toHaveBeenCalledWith(
      ExitCodes.FATAL_AUTHENTICATION_ERROR,
    );
  ...
  describe('JSON output mode', () => {
    ...
    it(`prints JSON error when enforced auth mismatches current auth and exits with code ${ExitCodes.FATAL_AUTHENTICATION_ERROR}`, async () => {
      ...
      );
      {
        ...
        expect(payload.error.code).toBe(ExitCodes.FATAL_AUTHENTICATION_ERROR);
        expect(payload.error.message).toContain(
          "The enforced authentication type is 'onyx-api-key', but the current type is 'oauth-personal'. Please re-authenticate with the correct type.",
        );
      }
    });
