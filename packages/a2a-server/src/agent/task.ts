...
export class Task {
  ...
  private constructor(
    ...
    this.onyxClient = loopContext.onyxClient;
  ...
  // state managed within the @onyx-cli/core module.
  ...
  async acceptAgentMessage(event: ServerOnyxStreamEvent): Promise<void> {
    const stateChange: StateChange = {
      kind: CoderAgentEvent.StateChangeEvent,
    };
    const traceId =
    ...
    switch (event.type) {
      ...
      case OnyxEventType.ToolCallResponse:
        // This event type from ServerOnyxStreamEvent might be for when LLM *generates* a tool response part.
      ...
      default: {
        ...
        let errorEvent: ServerOnyxErrorEvent | undefined;
        if (
          event.type === OnyxEventType.Error &&
          event.value &&
  ...
  async *sendCompletedToolsToLlm(
    ...
    yield* this.onyxClient.sendMessageStream(
      llmParts,
      aborted,
