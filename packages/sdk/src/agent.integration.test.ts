...
import { OnyxCliAgent } from './agent.js';
...
describe('OnyxCliAgent Integration', () => {
  beforeEach(() => {
  ...
  it('throws on invalid instructions', () => {
    ...
    expect(() => new OnyxCliAgent({} as any).session()).not.toThrow();

    expect(() =>
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      new OnyxCliAgent({ instructions: 123 as any }).session(),
