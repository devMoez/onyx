...
describe('ModelAvailabilityService', () => {
  ...
  describe('prefix normalization', () => {
    it('treats prefixed and non-prefixed models as identical when marking terminal', () => {
      service.markTerminal('models/onyx-3.1-pro-preview', 'quota');

      // Checking the non-prefixed version should show it as unavailable
      expect(service.snapshot('onyx-3.1-pro-preview')).toEqual({
        available: false,
        reason: 'quota',
      });

      // Checking the prefixed version should also show it as unavailable
      expect(service.snapshot('models/onyx-3.1-pro-preview')).toEqual({
        available: false,
        reason: 'quota',
      });
    });
    ...
    it('treats prefixed and non-prefixed models as identical when selecting', () => {
      service.markTerminal('onyx-3-flash-preview', 'quota');
      ...
      const result = service.selectFirstAvailable([
        'models/onyx-3-flash-preview',
        'onyx-3.1-pro-preview',
      ]);

      expect(result.selectedModel).toBe('onyx-3.1-pro-preview');
      expect(result.skipped).toEqual([
        { model: 'onyx-3-flash-preview', reason: 'quota' },
      ]);
    });

    it('treats prefixed and non-prefixed models as identical when marking healthy', () => {
      service.markTerminal('onyx-3-flash-preview', 'quota');
      service.markHealthy('models/onyx-3-flash-preview');

      expect(service.snapshot('onyx-3-flash-preview')).toEqual({
        available: true,
      });
    });
  });
