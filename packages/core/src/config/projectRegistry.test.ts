...
describe('ProjectRegistry', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'onyx-registry-test-'));
  ...
  it('handles collisions with unique suffixes', async () => {
    ...
    const id1 = await registry.getShortId(path.join(tempDir, 'one', 'onyx'));
    const id2 = await registry.getShortId(path.join(tempDir, 'two', 'onyx'));
    const id3 = await registry.getShortId(
      path.join(tempDir, 'three', 'onyx'),
    );

    expect(id1).toBe('onyx');
    expect(id2).toBe('onyx-1');
    expect(id3).toBe('onyx-2');
  });
  ...
  it('handles collisions if a slug is taken on disk by another project', async () => {
    // 1. project-y takes 'onyx' on disk
    ...
    // 2. project-z tries to get shortId for 'onyx'
    const registry = new ProjectRegistry(registryPath, [baseDir1]);
    await registry.initialize();
    const projectZ = normalizePath(path.join(tempDir, 'onyx'));
    const shortId = await registry.getShortId(projectZ);

    // 3. It should avoid 'onyx' and pick 'onyx-1' (or similar)
    expect(shortId).not.toBe('onyx');
    expect(shortId).toBe('onyx-1');
  });
