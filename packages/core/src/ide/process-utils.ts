 ...
 * process).
 *
 * This function can be overridden by setting the `ONYX_CLI_IDE_PID`
 * environment variable. This is useful for launching Onyx CLI in a
 * standalone terminal while still connecting to an IDE instance.
 *
 * If `ONYX_CLI_IDE_PID` is set, the function uses that PID and fetches
 * the command for it.
 *
...
export async function getIdeProcessInfo(): Promise<{
  ...
  if (process.env['ONYX_CLI_IDE_PID']) {
    const idePid = parseInt(process.env['ONYX_CLI_IDE_PID'], 10);
