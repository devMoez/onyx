...
export function validateWorkspacePath(
  ...
  if (!isWithinWorkspace) {
    return {
      isValid: false,
      error: `Directory mismatch. Onyx CLI is running in a different location than the open workspace in the IDE. Please run the CLI from one of the following directories: ${ideWorkspacePaths.join(
        ', ',
      )}`,
    };
  }
...
export function getPortFromEnv(): string | undefined {
  const port = process.env['ONYX_CLI_IDE_SERVER_PORT'];
  if (!port) {
    return undefined;
  }
  return port;
...
export function getStdioConfigFromEnv(): StdioConfig | undefined {
  const command = process.env['ONYX_CLI_IDE_SERVER_STDIO_COMMAND'];
  if (!command) {
    return undefined;
  }

  const argsStr = process.env['ONYX_CLI_IDE_SERVER_STDIO_ARGS'];
  let args: string[] = [];
  if (argsStr) {
    try {
      ...
      if (Array.isArray(parsedArgs)) {
        ...
        logger.error(
          'ONYX_CLI_IDE_SERVER_STDIO_ARGS must be a JSON array string.',
        );
      }
    } catch (e) {
      logger.error('Failed to parse ONYX_CLI_IDE_SERVER_STDIO_ARGS:', e);
    }
  }
...
const IDE_SERVER_FILE_REGEX = /^onyx-ide-server-(\d+)-\d+\.json$/;
...
> {
  ...
  try {
    ...
    // /^onyx-ide-server-${pid}-\d+\.json$/. If multiple IDE
    // windows are open, multiple files matching the pattern are expected to
    // exist.
  }

  const portFileDir = path.join(os.tmpdir(), 'onyx', 'ide');
  let portFiles;
  try {
