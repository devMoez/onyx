...
describe('ide-connection-utils', () => {
  beforeEach(() => {
    ...
    vi.stubEnv('ONYX_CLI_IDE_WORKSPACE_PATH', '/test/workspace');
    vi.stubEnv('ONYX_CLI_IDE_SERVER_PORT', '');
    vi.stubEnv('ONYX_CLI_IDE_SERVER_STDIO_COMMAND', '');
    vi.stubEnv('ONYX_CLI_IDE_SERVER_STDIO_ARGS', '');
    vi.stubEnv('ONYX_CLI_IDE_AUTH_TOKEN', '');
  ...
  describe('getConnectionConfigFromFile', () => {
    it('should return config from the specific pid file if it exists', async () => {
      ...
      expect(fs.promises.readFile).toHaveBeenCalledWith(
        path.join('/tmp', 'onyx', 'ide', 'onyx-ide-server-12345.json'),
        'utf8',
      );
    });
    ...
    it('should find and parse a single config file with the new naming scheme', async () => {
      ...
      (
        vi.mocked(fs.promises.readdir) as Mock<
          (path: fs.PathLike) => Promise<string[]>
        >
      ).mockResolvedValue(['onyx-ide-server-12345-123.json']);
      vi.mocked(fs.promises.readFile).mockResolvedValue(JSON.stringify(config));
      ...
      expect(result).toEqual(config);
      expect(fs.promises.readFile).toHaveBeenCalledWith(
        path.join('/tmp', 'onyx', 'ide', 'onyx-ide-server-12345-123.json'),
        'utf8',
      );
    });
    ...
    it('should filter out configs with invalid workspace paths', async () => {
      ...
      (
        ...
        'onyx-ide-server-12345-111.json',
        'onyx-ide-server-12345-222.json',
      ]);
    ...
    it('should fall back to a different PID if it matches the current workspace', async () => {
      ...
      (
        vi.mocked(fs.promises.readdir) as Mock<
          (path: fs.PathLike) => Promise<string[]>
        >
      ).mockResolvedValue([`onyx-ide-server-${otherPid}-111.json`]);
      ...
      expect(fs.promises.readFile).toHaveBeenCalledWith(
        path.join(
          ...
          'onyx',
          'ide',
          `onyx-ide-server-${otherPid}-111.json`,
        ),
        'utf8',
      );
    });
    ...
    it('should prioritize the target PID over other PIDs', async () => {
      ...
      (
        ...
        `onyx-ide-server-${otherPid}-1.json`,
        `onyx-ide-server-${targetPid}-1.json`,
      ]);
    ...
    it('should prioritize an alive process over a dead one', async () => {
      ...
      (
        ...
        `onyx-ide-server-${deadPid}-1.json`,
        `onyx-ide-server-${alivePid}-1.json`,
      ]);
    ...
    it('should prioritize the largest PID (newest) among alive processes', async () => {
      ...
      (
        ...
        `onyx-ide-server-${oldPid}-1.json`,
        `onyx-ide-server-${newPid}-1.json`,
      ]);
    ...
    it('should prioritize the config matching the port from the environment variable', async () => {
      vi.stubEnv('ONYX_CLI_IDE_SERVER_PORT', '2222');
      const config1 = { port: '1111', workspacePath: '/test/workspace' };
    ...
    it('should ignore files with invalid names', async () => {
      ...
      (
        ...
        'onyx-ide-server-12345-111.json', // valid
        'not-a-config-file.txt', // invalid
        'onyx-ide-server-asdf.json', // invalid
      ]);
      vi.mocked(fs.promises.readFile).mockResolvedValueOnce(
        JSON.stringify(validConfig),
      );
      ...
      expect(result).toEqual(validConfig);
      expect(fs.promises.readFile).toHaveBeenCalledWith(
        path.join('/tmp', 'onyx', 'ide', 'onyx-ide-server-12345-111.json'),
        'utf8',
      );
      expect(fs.promises.readFile).not.toHaveBeenCalledWith(
        path.join('/tmp', 'onyx', 'ide', 'not-a-config-file.txt'),
        'utf8',
      );
    });

    it('should match env port string to a number port in the config', async () => {
      vi.stubEnv('ONYX_CLI_IDE_SERVER_PORT', '3333');
      const config1 = { port: 1111, workspacePath: '/test/workspace' };
