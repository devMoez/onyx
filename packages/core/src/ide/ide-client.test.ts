...
describe('IdeClient', () => {
  ...
  beforeEach(async () => {
    ...
    process.env['ONYX_CLI_IDE_WORKSPACE_PATH'] = '/test/workspace';
    delete process.env['ONYX_CLI_IDE_SERVER_PORT'];
    delete process.env['ONYX_CLI_IDE_SERVER_STDIO_COMMAND'];
    delete process.env['ONYX_CLI_IDE_SERVER_STDIO_ARGS'];
    delete process.env['ONYX_CLI_IDE_AUTH_TOKEN'];
  ...
  describe('authentication', () => {
    ...
    it('should connect with an auth token from environment variable if config file is missing', async () => {
      ...
      vi.mocked(validateWorkspacePath).mockReturnValue({ isValid: true });
      vi.mocked(getPortFromEnv).mockReturnValue('9090');
      process.env['ONYX_CLI_IDE_AUTH_TOKEN'] = 'env-auth-token';
