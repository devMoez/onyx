...
describe('Core System Prompt (prompts.ts)', () => {
  ...
  beforeEach(() => {
    ...
    vi.stubEnv('ONYX_SYSTEM_MD', undefined);
    vi.stubEnv('ONYX_WRITE_SYSTEM_MD', undefined);
    const mockRegistry = {
      getAllToolNames: vi
        .fn()
  ...
  it('should use chatty system prompt for preview model', () => {
    vi.mocked(mockConfig.getActiveModel).mockReturnValue(PREVIEW_GEMINI_MODEL);
    const prompt = getCoreSystemPrompt(mockConfig);
    expect(prompt).toContain('You are Onyx CLI, an interactive CLI agent'); // Check for core content
    expect(prompt).toContain('- **User Hints:**');
  ...
  it('should append userMemory with separator when provided', () => {
    ...
    expect(prompt).toContain('You are Onyx CLI, an interactive CLI agent'); // Ensure base prompt follows
  ...
  it.each([
    ...
    [
      undefined,
      'You are Onyx CLI, an interactive CLI agent',
  ...
  describe('ONYX_SYSTEM_MD environment variable', () => {
    it.each(['false', '0'])(
      'should use default prompt when ONYX_SYSTEM_MD is "%s"',
      (value) => {
        vi.stubEnv('ONYX_SYSTEM_MD', value);
        const prompt = getCoreSystemPrompt(mockConfig);
    ...
    it('should throw error if ONYX_SYSTEM_MD points to a non-existent file', () => {
      const customPath = '/non/existent/path/system.md';
      vi.stubEnv('ONYX_SYSTEM_MD', customPath);
    ...
    it.each(['true', '1'])(
      'should read from default path when ONYX_SYSTEM_MD is "%s"',
      (value) => {
        const defaultPath = path.resolve(path.join(ONYX_DIR, 'system.md'));
        vi.stubEnv('ONYX_SYSTEM_MD', value);
    ...
    it('should read from custom path when ONYX_SYSTEM_MD provides one, preserving case', () => {
    ...
    it('should expand tilde in custom path when ONYX_SYSTEM_MD is set', () => {
  ...
  describe('ONYX_WRITE_SYSTEM_MD environment variable', () => {
    it.each(['false', '0'])(
      'should not write to file when ONYX_WRITE_SYSTEM_MD is "%s"',
      (value) => {
        vi.stubEnv('ONYX_WRITE_SYSTEM_MD', value);
    ...
    it.each(['true', '1'])(
      'should write to default path when ONYX_WRITE_SYSTEM_MD is "%s"',
      (value) => {
        const defaultPath = path.resolve(path.join(ONYX_DIR, 'system.md'));
        vi.stubEnv('ONYX_WRITE_SYSTEM_MD', value);
        getCoreSystemPrompt(mockConfig);
    ...
    it('should write to custom path when ONYX_WRITE_SYSTEM_MD provides one', () => {
      const customPath = path.resolve('/custom/path/system.md');
      vi.stubEnv('ONYX_WRITE_SYSTEM_MD', customPath);
      getCoreSystemPrompt(mockConfig);
      expect(fs.writeFileSync).toHaveBeenCalledWith(
        customPath,
        expect.any(String),
    ...
    it.each([
      ...
      'should expand tilde in custom path when ONYX_WRITE_SYSTEM_MD is "%s"',
