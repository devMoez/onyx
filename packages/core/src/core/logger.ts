...
export class Logger {
  private onyxDir: string | undefined;
  ...
  async initialize(): Promise<void> {
    ...
    this.onyxDir = this.storage.getProjectTempDir();
    this.logFilePath = path.join(this.onyxDir, LOG_FILE_NAME);

    try {
      await fs.mkdir(this.onyxDir, { recursive: true });
  ...
  private _checkpointPath(tag: string): string {
    ...
    return path.join(this.onyxDir, `checkpoint-${encodedTag}.json`);
  }
  ...
  private async _getCheckpointPath(tag: string): Promise<string> {
    ...
    const oldPath = path.join(this.onyxDir!, `checkpoint-${tag}.json`);
    try {
  ...
  async deleteCheckpoint(tag: string): Promise<boolean> {
    if (!this.initialized || !this.onyxDir) {
      debugLogger.error(
