...
describe('modifyWithEditor', () => {
  ...
  it('should create temp files with correct naming with extension', async () => {
    ...
    expect(mockOpenDiff).toHaveBeenCalledOnce();
    const [oldFilePath, newFilePath] = mockOpenDiff.mock.calls[0];
    expect(oldFilePath).toMatch(/onyx-cli-modify-test-file-old-\d+\.txt$/);
    expect(newFilePath).toMatch(/onyx-cli-modify-test-file-new-\d+\.txt$/);

    const diffDirPrefix = path.join(os.tmpdir(), 'onyx-cli-tool-modify-');
  ...
  it('should create temp files with correct naming without extension', async () => {
    ...
    expect(mockOpenDiff).toHaveBeenCalledOnce();
    const [oldFilePath, newFilePath] = mockOpenDiff.mock.calls[0];
    expect(oldFilePath).toMatch(/onyx-cli-modify-test-file-old-\d+$/);
    expect(newFilePath).toMatch(/onyx-cli-modify-test-file-new-\d+$/);
