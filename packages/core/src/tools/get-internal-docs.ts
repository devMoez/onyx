...
async function getDocsRoot(): Promise<string> {
  ...
  throw new Error('Could not find Onyx CLI documentation directory.');
}
  ...
  async execute({ abortSignal: _signal }: ExecuteOptions): Promise<ToolResult> {
    try {
      ...
      if (!this.params.path) {
        ...
        const resultContent = `Available Onyx CLI documentation files:\n\n${fileList}`;
...
/**
 * A tool that provides access to Onyx CLI's internal documentation.
