...
describe('telemetry/config helpers', () => {
  ...
  describe('resolveTelemetrySettings', () => {
    ...
    it('uses env over settings and argv over env', async () => {
      ...
      const env = {
        ONYX_TELEMETRY_ENABLED: '1',
        ONYX_TELEMETRY_TARGET: 'gcp',
        ONYX_TELEMETRY_OTLP_ENDPOINT: 'http://env:4317',
        ONYX_TELEMETRY_OTLP_PROTOCOL: 'http',
        ONYX_TELEMETRY_LOG_PROMPTS: 'true',
        ONYX_TELEMETRY_OUTFILE: 'env.log',
        ONYX_TELEMETRY_USE_COLLECTOR: 'true',
    ...
    it('resolves useCliAuth from env', async () => {
      const env = {
        ONYX_TELEMETRY_USE_CLI_AUTH: 'true',
      };
    ...
    it('falls back to OTEL_EXPORTER_OTLP_ENDPOINT when ONYX var is missing', async () => {
      const settings = {};
      const env = {
    ...
    it('throws on unknown protocol values', async () => {
      const env = { ONYX_TELEMETRY_OTLP_PROTOCOL: 'unknown' } as Record<
        string,
        string
      >;
    ...
    it('throws on unknown target values', async () => {
      const env = { ONYX_TELEMETRY_TARGET: 'unknown' } as Record<
        string,
        string
      >;
      await expect(resolveTelemetrySettings({ env })).rejects.toThrow(
