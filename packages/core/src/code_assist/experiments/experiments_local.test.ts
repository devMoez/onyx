...
describe('experiments with ONYX_EXP', () => {
  let mockServer: CodeAssistServer;

  beforeEach(() => {
    vi.resetModules();
    vi.clearAllMocks();
    process.env['ONYX_EXP'] = ''; // Clear env var
  ...
  afterEach(() => {
    delete process.env['ONYX_EXP'];
  });

  it('should read experiments from local file if ONYX_EXP is set', async () => {
    process.env['ONYX_EXP'] = '/tmp/experiments.json';
  ...
  it('should fall back to server if reading file fails', async () => {
    process.env['ONYX_EXP'] = '/tmp/missing.json';
  ...
  it('should return empty if no server and no ONYX_EXP', async () => {
    const { getExperiments } = await import('./experiments.js');
    const experiments = await getExperiments(undefined);
  ...
  it('should fallback to server if file has invalid structure', async () => {
    process.env['ONYX_EXP'] = '/tmp/invalid.json';
    const mockFileContent = JSON.stringify({
