...
describe('sanitizeEnvironment', () => {
  ...
  it('should allow variables prefixed with ONYX_CLI_', () => {
    const env = {
      ONYX_CLI_FOO: 'bar',
      ONYX_CLI_BAZ: 'qux',
    };
  ...
  describe('value-first security: secret values must be caught even for allowed variable names', () => {
    ...
    it('should NOT redact ONYX_CLI_ variables even if their value looks like a secret (fully trusted)', () => {
      const env = {
        ONYX_CLI_INTERNAL: 'ghp_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',
      };
      const sanitized = sanitizeEnvironment(env, EMPTY_OPTIONS);
      expect(sanitized).toEqual(env);
    });
