...
import { OnyxEventType, type ServerOnyxStreamEvent } from '../core/turn.js';
import {
  logLoopDetected,
...
export class LoopDetectionService {
   ...
   */
  addAndCheck(event: ServerOnyxStreamEvent): LoopDetectionResult {
    if (
    ...
    if (isLoop) {
      ...
      this.lastLoopType =
        event.type === OnyxEventType.ToolCallRequest
  ...
  private async checkForLoopWithLLM(signal: AbortSignal): Promise<{
    ...
    const recentHistory = this.context.onyxClient
      .getHistory()
