 ...
 * from within Onyx CLI.
 */
export const ONYX_CLI_IDENTIFICATION_ENV_VAR = 'ONYX_CLI';

/**
 * The value of {@link ONYX_CLI_IDENTIFICATION_ENV_VAR}
 */
export const ONYX_CLI_IDENTIFICATION_ENV_VAR_VALUE = '1';
...
export class ShellExecutionService {
  ...
  private static async prepareExecution(
    ...
    const baseEnv: Record<string, string | undefined> = {
      ...sanitizedEnv,
      [ONYX_CLI_IDENTIFICATION_ENV_VAR]:
        ONYX_CLI_IDENTIFICATION_ENV_VAR_VALUE,
      TERM: 'xterm-256color',
  ...
  private static async childProcessFallback(
    ...
    try {
      ...
      const handleExit = (
        ...
        if (state.truncated) {
          const truncationMessage = `\n[ONYX_CLI_WARNING: Output truncated. The buffer is limited to ${
    ...
    try {
      ...
      if (error?.message?.includes('posix_spawnp failed')) {
        onOutputEvent({
          type: 'data',
          chunk:
            '[ONYX_CLI_WARNING] PTY execution failed, falling back to child_process. This may be due to sandbox restrictions.\n',
        });
        throw e;
      } else {
        return {
