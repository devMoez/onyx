...
describe('ModelConfigService', () => {
  ...
  it('should apply the most specific override rule', () => {
    const config: ModelConfigServiceConfig = {
      ...
      overrides: [
        {
          match: { model: 'onyx-pro' },
          modelConfig: { generateContentConfig: { temperature: 0.5 } },
        },
        {
          match: { model: 'onyx-pro', overrideScope: 'my-agent' },
    ...
    expect(resolved.model).toBe('onyx-pro');
  ...
  it('should use the last override in case of a tie in specificity', () => {
    ...
    const service = new ModelConfigService(config);
    const resolved = service.getResolvedConfig({ model: 'onyx-pro' });
  ...
  it('should merge settings from global, alias, and multiple matching overrides', () => {
    const config: ModelConfigServiceConfig = {
      ...
      overrides: [
        ...
        {
          match: { model: 'onyx-test-model', overrideScope: 'test-agent' },
          modelConfig: {
            generateContentConfig: {
              temperature: 0.2,
            },
  ...
  describe('alias inheritance', () => {
    ...
    describe('abstract aliases', () => {
      it('should allow an alias to extend an abstract alias without a model', () => {
        ...
        expect(resolved.model).toBe('onyx-1.5-pro-latest');
        expect(resolved.generateContentConfig).toEqual({
          temperature: 0.1,
          topP: 0.9,
        });
    ...
    it('should throw an error if the alias chain is too deep', () => {
      ...
      for (let i = 0; i < 101; i++) {
        aliases[`alias-${i}`] = {
          ...
          modelConfig: i === 100 ? { model: 'onyx-pro' } : {},
        };
      }
  ...
  describe('runtime aliases', () => {
    it('should resolve a simple runtime-registered alias', () => {
      ...
      expect(resolved.model).toBe('onyx-runtime-model');
      expect(resolved.generateContentConfig).toEqual({
        temperature: 0.123,
      });
    });
  ...
  describe('custom aliases', () => {
    it('should resolve a custom alias', () => {
      ...
      expect(resolved.model).toBe('onyx-custom');
      expect(resolved.generateContentConfig).toEqual({
        temperature: 0.9,
      });
    });
    ...
    it('should allow custom aliases to override built-in aliases', () => {
      ...
      expect(resolved.model).toBe('onyx-custom-override');
      expect(resolved.generateContentConfig).toEqual({
        temperature: 0.1,
      });
    });
  ...
  describe('getAvailableModelOptions', () => {
    it('should filter out Pro models when hasAccessToProModel is false', () => {
      const config: ModelConfigServiceConfig = {
        modelDefinitions: {
          'onyx-3-pro': { isVisible: true, tier: 'pro' },
          'onyx-3-flash': { isVisible: true, tier: 'flash' },
        },
      };
      const service = new ModelConfigService(config);
      ...
      expect(options.map((o) => o.modelId)).not.toContain('onyx-3-pro');
      expect(options.map((o) => o.modelId)).toContain('onyx-3-flash');
    });
    ...
    it('should include Pro models when hasAccessToProModel is true or undefined', () => {
      ...
      expect(optionsWithTrue.map((o) => o.modelId)).toContain('onyx-3-pro');
