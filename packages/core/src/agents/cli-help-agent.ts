...
const CliHelpReportSchema = z.object({
  answer: z
    .string()
    .describe('The detailed answer to the user question about Onyx CLI.'),
  sources: z
    .array(z.string())
...
/**
 * An agent specialized in answering questions about Onyx CLI itself,
...
export const CliHelpAgent = (
  ...
  description:
    'Specialized agent for answering questions about the Onyx CLI application. Invoke this agent for questions regarding CLI features, configuration schemas (e.g., policies), or instructions on how to create custom subagents. It queries internal documentation to provide accurate usage guidance.',
  inputConfig: {
    inputSchema: {
      type: 'object',
      properties: {
        question: {
          type: 'string',
          description: 'The specific question about Onyx CLI.',
        },
      },
      required: ['question'],
    },
  ...
  promptConfig: {
    query:
      'Your task is to answer the following question about Onyx CLI:\n' +
      '<question>\n' +
      '${question}\n' +
      '</question>',
    systemPrompt:
      "You are **CLI Help Agent**, an expert on Onyx CLI. Your purpose is to provide accurate information about Onyx CLI's features, configuration, and current state.\n\n" +
      '### Runtime Context\n' +
