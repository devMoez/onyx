    ...
    const batchPromises = batch.map(
      async (filePath): Promise<OnyxFileContent> => {
        try {
          ...
          if (isEISDIR) {
            // A directory exists where a onyx.md file is expected.
            // This is valid in some project structures (e.g. a folder named
            // onyx.md held for organisational purposes) — skip it silently
            // instead of surfacing a confusing warning to the user.
            debugLogger.debug(
              '[DEBUG] [MemoryDiscovery] Skipping directory at onyx.md path:',
              filePath,
            );
          } else {
            const isTestEnv =
...
export async function getUserProjectMemoryPaths(
  ...
  try {
    ...
    // Fall back to the legacy private onyx.md file if the project has not
...
/**
 * Traverses upward from startDir to stopDir, finding all onyx.md variants.
 *
...
export async function loadJitSubdirectoryMemory(
  ...
  // path like /app/src/file.ts/onyx.md.
