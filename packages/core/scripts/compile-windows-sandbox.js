...
/**
 * Compiles the onyxSandbox C# helper on Windows.
...
function compileWindowsSandbox() {
  ...
  const srcHelperPath = path.resolve(
    __dirname,
    '../src/sandbox/windows/onyxSandbox.exe',
  );
  const distHelperPath = path.resolve(
    __dirname,
    '../dist/src/sandbox/windows/onyxSandbox.exe',
  );
  const sourcePath = path.resolve(
    __dirname,
    '../src/sandbox/windows/onyxSandbox.cs',
  );
  ...
  if (result.status === 0) {
    console.log('Successfully compiled onyxSandbox.exe to src');
    ...
    if (fs.existsSync(distDir)) {
      ...
      console.log('Successfully copied onyxSandbox.exe to dist');
    }
  } else {
