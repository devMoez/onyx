...
const getPortFromMock = (
  ...
  const port = vi
    .mocked(replaceMock)
    .mock.calls.find((call) => call[0] === 'ONYX_CLI_IDE_SERVER_PORT')?.[1];
...
describe('IDEServer', () => {
  ...
  it('should set environment variables and workspace path on start with multiple folders', async () => {
    ...
    const expectedPortFile = path.join(
      ...
      'onyx',
      'ide',
      `onyx-ide-server-${process.ppid}-${port}.json`,
    );
    ...
    expect(fs.mkdir).toHaveBeenCalledWith(path.join('/tmp', 'onyx', 'ide'), {
      recursive: true,
    });
