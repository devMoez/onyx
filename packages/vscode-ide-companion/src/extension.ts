...
const INFO_MESSAGE_SHOWN_KEY = 'onyxCliInfoMessageShown';
export const DIFF_SCHEME = 'onyx-diff';
  ...
  try {
    ...
    if (
      ...
      const selection = await vscode.window.showInformationMessage(
        `A new version (${latestVersion}) of the Onyx CLI Companion extension is available.`,
...
export async function activate(context: vscode.ExtensionContext) {
  logger = vscode.window.createOutputChannel('Onyx CLI IDE Companion');
  ...
  if (
    ...
    void vscode.window.showInformationMessage(
      'Onyx CLI Companion extension successfully installed.',
    );
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand('onyx-cli.runOnyxCLI', async () => {
      const workspaceFolders = vscode.workspace.workspaceFolders;
      if (!workspaceFolders || workspaceFolders.length === 0) {
        vscode.window.showInformationMessage(
          'No folder open. Please open a folder to run Onyx CLI.',
        );
        return;
      }
      ...
      if (workspaceFolders.length === 1) {
        ...
        selectedFolder = await vscode.window.showWorkspaceFolderPick({
          placeHolder: 'Select a folder to run Onyx CLI in',
        });
      }
      ...
      if (selectedFolder) {
        ...
        const terminal = vscode.window.createTerminal({
          name: `Onyx CLI (${selectedFolder.name})`,
          cwd: selectedFolder.uri.fsPath,
        });
        terminal.show();
        terminal.sendText(onyxCmd);
      }
    }),
    vscode.commands.registerCommand('onyx-cli.showNotices', async () => {
