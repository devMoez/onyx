...
} from '@onyx/core/src/ide/types.js';
...
const IDE_SERVER_PORT_ENV_VAR = 'ONYX_CLI_IDE_SERVER_PORT';
const IDE_WORKSPACE_PATH_ENV_VAR = 'ONYX_CLI_IDE_WORKSPACE_PATH';
const IDE_AUTH_TOKEN_ENV_VAR = 'ONYX_CLI_IDE_AUTH_TOKEN';
    ...
    return new Promise((resolve) => {
      ...
      this.server = app.listen(0, '127.0.0.1', async () => {
        ...
        if (address && typeof address !== 'string') {
          ...
          try {
            const portDir = path.join(tmpdir(), 'onyx', 'ide');
            await fs.mkdir(portDir, { recursive: true });
            portFile = path.join(
              portDir,
              `onyx-ide-server-${process.ppid}-${this.port}.json`,
            );
...
const createMcpServer = (
  ...
  const server = new McpServer(
    {
      name: 'onyx-cli-companion-mcp-server',
      version: '1.0.0',
    },
