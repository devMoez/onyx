...
function checkRelease() {
  try {
    ...
    const bucketUri = 'gs://onyx-cli-vscode-extension/release/1p/signed';
    ...
    const vsixFiles = files.filter((file) =>
      /signed-onyx-cli-vscode-ide-companion-\d+\.\d+\.\d+-[a-f0-9]{7}\.vsix$/.test(
        file,
      ),
    );
    ...
    const match =
      /signed-onyx-cli-vscode-ide-companion-(\d+\.\d+\.\d+)-([a-f0-9]{7})\.vsix$/.exec(
        fileName,
      );
